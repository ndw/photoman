...
let $params := rest:process-request(endpoints:request("/ajax/change-photo-jeanne.xqy"))
...
return
  ...
  then
    (if ($xml/npl:jeanne)
     then
       xdmp:node-replace($xml/npl:jeanne, <npl:jeanne>{$value}</npl:jeanne>)
     else
       xdmp:node-insert-child($xml, <npl:jeanne>{$value}</npl:jeanne>),
