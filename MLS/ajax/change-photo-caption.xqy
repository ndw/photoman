...
let $params := rest:process-request(endpoints:request("/ajax/change-photo-caption.xqy"))
...
return
  ...
  then
    (if ($xml/npl:caption)
     then
       xdmp:node-replace($xml/npl:caption, <npl:caption>{$value}</npl:caption>)
     else
       xdmp:node-insert-child($xml, <npl:caption>{$value}</npl:caption>),
