...
let $params := rest:process-request(endpoints:request("/ajax/add-tag.xqy"))
...
return
  if (u:admin())
  then
    if (f:tag-valid($photo, $value))
    then
      (xdmp:node-insert-before($photo/npl:images, <npl:tag>{$value}</npl:tag>),
